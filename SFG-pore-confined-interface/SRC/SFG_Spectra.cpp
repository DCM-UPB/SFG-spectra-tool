...
int main(int argc, char** argv)
{
  ...
  for(unsigned int t = from;t < to;++t)
    { 
      ...
      for(unsigned int i = 0;i < N;++i)
        { 
          if( sqrt(O_coord_z[N*t+i]*O_coord_z[N*t+i] + O_coord_x[N*t+i]* O_coord_x[N*t+i]) >  R1  &&
              sqrt(O_coord_z[N*t+i]*O_coord_z[N*t+i] + O_coord_x[N*t+i]* O_coord_x[N*t+i]) < R2 )
            { 
              mean_ += 1; 
              for(unsigned int t_ = 0;t_ < tcfl;++t_)
                {
